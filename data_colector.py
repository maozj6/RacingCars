...
def origin():
    ...
    parser.add_argument('-e', '--episodes', type=int, default=100, help='The number of episodes should the model plays.')
    ...
    for e in range(play_episodes):
        ...
        while True:
                ...
                if frameCounter!=0 or frameCounter!=1:
    ...
    np.save("datas/safe.npy", fr)
    np.save("datas/label.npy", safeties)

    # dataframe = pd.DataFrame({'Speed': Speed,})
    dataframe = pd.DataFrame({ 'Ave_Color_Value': Ave_Color_Value,'Safety': Safety})

    dataframe.to_csv("./safe3.csv",index=False,sep=',')
